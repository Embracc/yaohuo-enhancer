...
(function() {
    ...
    function ensureSettingsBtn() {
        ...
        function openSettings() {
            ...
            var html = '<div style="padding:14px 16px;background:linear-gradient(135deg,#1abc9c,#16a085);color:#fff;font-size:15px;font-weight:bold;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:2;border-radius:14px 14px 0 0"><span>⚙ 设置 <small style="opacity:.8;font-weight:normal;font-size:11px">v0.9.184</small></span><span class="yh-settings-close" style="cursor:pointer;font-size:22px;line-height:1;padding:0 4px;opacity:.8;transition:opacity .15s">&times;</span></div><div style="padding:6px 14px 14px">';
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (raw.indexOf('今天') >= 0) {
                ...
                if (m) {
                    ...
                    if (pm && pd) {
                        var postMs = Date.UTC(curYear, pm - 1, pd);
                        var nowMs = Date.UTC(curYear, curMon - 1, curDay);
                        // 如果算出来比当前日期大太多（跨年帖），回退到上一年
                        if (postMs > nowMs && (postMs - nowMs) > 86400000 * 30) {
                            postMs = Date.UTC(curYear - 1, pm - 1, pd);
                        }
    ...
    console.log('[YH] 初始化完成 v0.9.184 by Embrace/19299');
})();