...
(function() {
            ...
            onload: function(res2) {
                try {
                    ...
                    var baseMsg = function(extra) {
                        var s = L + '\u6635\u79f0' + R + (nick || '-')
                            + '\n' + L + 'ID' + R + idShow
                            + '\n' + L + '\u5996\u6676' + R + parseField(html2, '\u5996\u6676')
                            + '\n' + L + '\u7ecf\u9a8c' + R + parseField(html2, '\u7ecf\u9a8c')
    ...
    function shouldAutoCheckUpdate() {
        // 每次打开网页（脚本加载）可检一次；同页不重复。1 分钟内多次打开 = 多次检测。
    ...
    function showImageZoom(src) {
        ...
        img.src = src;
        img.style.cssText = 'max-width:90vw;max-height:90vh;object-fit:contain;border-radius:4px;box-shadow:0 4px 30px rgba(0,0,0,.5);transition:transform .2s ease;cursor:grab;';
        img.draggable = false;
